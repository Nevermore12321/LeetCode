...
```
4. 编写申请证书请求文件的 yaml 文件，并且创建 csr
...
```
5. 通过 kubernetes 的 master，apiserver 通过 john 的 csr  请求
...
- name: john
  user:
    ...
    client-key-data: LS0tLS1CRUd....
```
7. 至此，kubeconfig 文件就设置好了， 可以拷贝到其他机器上来通过 kubectl 来访问集群。
...
- `--authorization-mode=Node` :专门授权由 kubelet 发出的 API 请求。
    - Node授权器主要用于各个 node 上的 kubelet 访问 apiserver 时使用的，其他一般均由 RBAC 授权器来授权
