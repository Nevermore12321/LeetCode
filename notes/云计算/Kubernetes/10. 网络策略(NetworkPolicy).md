...
通过yaml文件创建 NetworkPolicy 资源，yaml文件模板为：
```
...
- `policyTypes` : 
    - 每个 NetworkPolicy 都包含一个 policyTypes 列表，其中包含 Ingress 或 Egress 或两者兼具。policyTypes 字段表示给定的策略是否应用于进入所选 Pod 的入口流量或者来自所选 Pod 的出口流量，或两者兼有。如果 NetworkPolicy 未指定 policyTypes 则默认情况下始终设置 Ingress，如果 NetworkPolicy 有任何出口规则的话则设置 Egress。
- `ingress` : 
      ...
      - from:
        ...
        - podSelector:
            matchLabels:
              role: client
    ```
    - 如果是 在同一目录下, 条件表示与，pod 有标签，并且该pod所在的 namespace 也有标签。
    ```
      ingress:
      - from:
        - namespaceSelector:
