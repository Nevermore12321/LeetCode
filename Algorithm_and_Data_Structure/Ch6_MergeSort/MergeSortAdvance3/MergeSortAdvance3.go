package MergeSortAdvance3

import "Algorithm_and_Data_Structure/Ch2_BaseSort/InsertionSort"

/*
归并排序的优化3：在merge过程中，每次开辟一个新的数组，在进行copy复制，很耗时
*/
...
func Sort[T int | int32 | int64 | float32 | float64 | uint | uint32 | uint64](arr []T) {
	temp := make([]T, len(arr))
	sort[T](arr, 0, len(arr)-1, temp)
}

// 归并排序的 递归部分，带有下标
func sort[T int | int32 | int64 | float32 | float64 | uint | uint32 | uint64](arr []T, l, r int, tmp []T) {
	// 基础 使用插入排序优化
	if r-l <= 15 {
	...
	// 递归左右部分
	sort[T](arr, l, mid-1, tmp)
	sort[T](arr, mid, r, tmp)
